...
12. Analyze browser-based Vocalize.ai example and integrate transcription configurations and User-Agent identification headers for maximum SDK alignment. [COMPLETED]


# COMPLETED ATOMIC STEPS
- Updated WebSocket setup payload to always initialize both `inputAudioTranscription` and `outputAudioTranscription` parameters (mirroring Google GenAI SDK behavior), allowing bidirectional audio transcripts during voice calls.
- Added `User-Agent: aistudio-build` HTTP header to the client's WebSocket request builder to mimic Google AI Studio SDK environments and prevent API gateway filter blocks.
- Updated `GominLiveManagerPayloadTest.kt` unit tests to verify the presence of bidirectional transcription parameters in voice setup configurations.
