...
1. **Input Payloads (Casing)**: Outgoing messages (sent by the client), such as the initial `setup` config, the streaming inputs `realtimeInput`, tool responses `toolResponse`, or the interruption `clientContent`, MUST use strict **camelCase** keys (`generationConfig`, `responseModalities`, `speechConfig`, `voiceConfig`, `prebuiltVoiceConfig`, `voiceName`, `googleSearch`, `functionDeclarations`, `clientContent`, `turnComplete`, `realtimeInput`, `mimeType`, `toolResponse`, `functionResponses`).
2. **Deprecated `mediaChunks` — Use `audio`/`video`/`text` Fields**: As of 2025+, `realtimeInput.mediaChunks[]` is **deprecated**. Audio must be sent via `realtimeInput.audio` (a single `Blob` with `mimeType` and `data`), video via `realtimeInput.video`, and text via `realtimeInput.text`. Never use `mediaChunks`.
3. **`clientContent` vs `realtimeInput.text` for Greeting**: `clientContent` is designed for incremental conversation history management with special semantics tied to `historyConfig.initialHistoryInClientContent`. For simple post-`setupComplete` text prompts (e.g., initial greeting trigger), use `realtimeInput.text` instead.
4. **`inputTranscription` / `outputTranscription` Placement**: Both `inputTranscription` and `outputTranscription` fields are part of `BidiGenerateContentServerContent` (i.e., inside the `serverContent` object), NOT at the top-level of the WebSocket message JSON.
5. **Output Payloads (Casing)**: Incoming messages (sent by the server), such as server content or tool calls, also arrive in **camelCase** (`serverContent`, `toolCall`, `functionCalls`).
6. **Teardown Mutex**: Always wrap session termination sequence in a thread-safe `synchronized` lock check (`if (!isSessionActive) return; isSessionActive = false`) to ensure system audio resources (`AudioRecord`, `AudioTrack`) and callback handlers are released exactly once, avoiding duplicate postings to the UI main thread and race crashes.
7. **OkHttpClient Reuse**: Never call `client.dispatcher.executorService.shutdown()` or `client.connectionPool.evictAll()` in `stopSession()` if the same `OkHttpClient` instance is reused across sessions. Killing the dispatcher breaks subsequent `newWebSocket()` calls.
8. **Playback Loop CPU Protection**: Before polling or feeding PCM blocks to `AudioTrack`, always explicitly assert that the track initialized successfully (`AudioTrack.STATE_INITIALIZED`). If initialization fails, terminate the playback thread and set active flags to false to prevent infinite loops and 100% CPU drain.
